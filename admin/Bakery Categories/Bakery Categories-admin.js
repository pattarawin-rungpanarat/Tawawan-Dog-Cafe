    ...
    deleteButtons.forEach(button => {
        button.addEventListener("click", function () {
            if (confirm("คุณแน่ใจหรือไม่ว่าต้องการลบเมนูนี้?")) {
                ...
                let bakeryCategories = JSON.parse(localStorage.getItem('bakeryCategories')) || [];
                bakeryCategories = bakeryCategories.filter(menu => menu.name !== menuName);
                localStorage.setItem('BakeryCategories', JSON.stringify(bakeryCategories));
            }
        });
    });
});
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadBakeryCategories() {
        const bakeryCategories = JSON.parse(localStorage.getItem('bakeryCategories')) || [];
        bakeryCategories.forEach(menu => {
    ...
    addButton.addEventListener("click", function () {
        ...
        const bakeryCategories = JSON.parse(localStorage.getItem('bakeryCategories')) || [];
        bakeryCategories.push({ imageUrl, name: menuName, price: menuPrice });
        localStorage.setItem('bakeryCategories', JSON.stringify(bakeryCategories));
